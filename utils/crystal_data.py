...
class ICDataset(Dataset):
    def __init__(
        ...
        self.oracleSendek = nn.Sequential(nn.Linear(5, 1, bias=True), nn.Sigmoid())
        with torch.no_grad():
            self.oracleSendek[0].weight = nn.Parameter(
                torch.tensor([0.18, -4.01, -0.47, 8.70, -2.17]).float()
            )
    ...
    def oracle_data(self, crystal):
        ...
        for e, elem in enumerate(crystal):
            ...
            if elem.specie.Z == A:
                rij0 = min(
                    [neigh[1] for neigh in crystal_neighs[e] if neigh.specie.Z == A]
                )
    ...
    def populate(self):
        ...
        fobj.write(
            "\t".join(["ID", "LLB", "SBI", "AFC", "LASD", "LLSD", "Psuperionic"]) + "\n"
        )
        skip_cifs = []
        for cif in self.cifs[:]:
            ...
            try:
                ...
                line = (
                    "\t".join([cif.split(".")[0]] + list(map(str, oracle)))
                    + "\t"
                    + str(y.data.item())
                    + "\n"
                )
    ...
    def __getitem__(self, idx):
        ...
        if self.subset:
            ...
            mat = torch.cat(
                (mat[:2], one_hot((mat[2] - 1).to(torch.int64), 230), mat[3:])
            )
        if self.transform:
            mat = (mat - self.transform["mean"]) / self.transform["std"]

        return (
            torch.nan_to_num(mat, nan=0.0).squeeze(0).squeeze(0),
            torch.tensor(y) >= 0.5,
        )

    def atom_info(self):
        warnings.filterwarnings("ignore")
        strucs = [Structure.from_file(osp.join(self.data_path, mp)) for mp in self.cifs]
        # tot_atoms = [len(s) for s in strucs]
        # num_atoms_type = [(i, n) for i, s in enumerate(strucs) for el, n in s.composition.to_reduced_dict.items()]
        num_diff_elem = [
            (i, len(s.composition.to_reduced_dict)) for i, s in enumerate(strucs)
        ]
        pos_max = max(num_diff_elem, key=lambda x: x[1])
        pos_min = min(num_diff_elem, key=lambda x: x[1])

        # print('Max Total', max(tot_atoms))
        # print('Min Total', min(tot_atoms))
        print("Max Element", pos_max[1], self.cifs[pos_max[0]])
        print("Min Element", pos_min[1], self.cifs[pos_min[0]])
...
def process_data(datapath, truecsv, avoid, avoidfile, featcsv, finalcsv):
    dataObj = CrystalDataset(
        datapath, truecsv, avoid, avoidfile, featcsv, finalcsv, None, True
    )
...
def verify_sendek():
    ...
    temp = CrystalDataset(
        "./data/li-ssb", "lissb.csv", skip=False, skip_fptr="skip.txt"
    )
    y = []
