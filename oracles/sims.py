...
def download(crystals, queryObj, save_dir):
    ...
    for i, c in enumerate(crystals):
        ...
        try:
            res = queryObj.query(
                criteria=crit, properties=["material_id", "structure"]
            )[0]
            struc = res["structure"]
...
def softbv_call(dirpath, fptr):
    ...
    try:
        cube_run = subprocess.run(
            ["softBV.x", "--gen-cube", fpath, "Li", str(1)], capture_output=True
        )
...
def verify():
    true = [0.07012716, 3.29437742e-15, 5.50841659e-06, 7.41314353e-05, 0.00144069]
    pred = [1.4e-2, 1e-6, 1e-13, 1e-7, 2.5e-4]
...
if __name__ == "__main__":
    ...
    # print(traj[0])
    print(
        calc.get_diffusion_coefficients()[0][0]
        * units.fs
        * 1e-1
        * 111.966
        * mol_den
